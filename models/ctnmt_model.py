...
class CtnmtModel(EncoderDecoderModel):
    """ Defines the Transformer models. """

    REGISTRY_NAME = "ctnmt_model"

    def __init__(self, args, bert_model, *largs, **kwargs):
        """ Initializes a sequence to sequence models.

        Args:
            args: A dict, containing the models configuration.
            bert_model: None or a bert models.
            name: The name of the models.options = tf.data.Options()
        """
        super(CtnmtModel, self).__init__(args, *largs, **kwargs)
        self._args = args
    ...
    def class_or_method_args():
        this_args = [x for x in super(CtnmtModel, CtnmtModel).class_or_method_args()
    ...
    def new(cls, args, src_meta, trg_meta, name=None):
        """ Builds a sequence to sequence models.

        Args:
            args: A dict containing all models parameters.
        ...
        with tf.name_scope(name or "ctnmt"):
            bert_model = Bert.new(get_hyper_parameters(args["bert_config"])["models.params"],
    ...
    def get_symbols_to_logits_fn(self, inputs, is_training, is_inference,
        ...
        Args:
            inputs: A dict of models inputs.
    ...
    def call(self, inputs, is_training=True):
        """ Forward pass of the sequence to sequence models.
...
@register_hparams_set("ctnmt_model_toy")
def ctnmt_model_toy():
    return CtnmtModel.build_model_args_by_name("ctnmt_toy")


@register_hparams_set("ctnmt_model_base")
def ctnmt_model_base():
    return CtnmtModel.build_model_args_by_name("ctnmt_model_base")


@register_hparams_set("ctnmt_model_big")
def ctnmt_model_big():
    return CtnmtModel.build_model_args_by_name("ctnmt_model_big")


build_ctnmt_model, register_ctnmt_model = setup_registry(CtnmtModel.REGISTRY_NAME, base_class=CtnmtModel,
                                                         create_fn="new",
                                                         verbose_creation=True)

register_ctnmt_model("ct_nmt")(CtnmtModel)
